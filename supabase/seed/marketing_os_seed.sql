...
-- Rodar APÓS supabase/migrations/marketing_os_schema.sql.
--
-- Preços preenchidos com os valores vigentes do site (08/07/2026).
-- Oferta vigente: preço de tabela, SEM promoção (a promo R$9,90 expirou em
-- 25/06/2026). Campos ainda marcados [REVISAR] podem ser editados depois
-- direto na tabela brand_kit — os agentes são instruídos a não prometer
-- nada que não esteja cadastrado aqui.
-- =============================================================================
...
('oferta_atual', '{
  "nome": "Preço de tabela (sem promoção vigente)",
  "publico": "todos",
  "mecanica": "assinatura mensal via cartão, boleto ou PIX; sem desconto de entrada no momento",
  "planos": {
    "essencial": {"usuarios": 1,  "preco": "R$ 129,90/mês", "resumo": "Cadastros + Nota Fiscal + Orçamento"},
    "standard":  {"usuarios": 3,  "preco": "R$ 199,90/mês", "resumo": "PDV completo + Financeiro + Boletos + MDF-e"},
    "premium":   {"usuarios": "ilimitados", "preco": "R$ 249,90/mês", "resumo": "Tudo + OS + Retaguarda offline + SPED + Grades"}
  },
  "regras_de_uso_em_copy": "NÃO existe promoção vigente: nunca prometer desconto, mensalidade reduzida ou condição especial. A promo ''primeira mensalidade R$ 9,90'' EXPIROU em 25/06/2026 — não citar. Urgência apenas legítima (ex.: prazos reais da Reforma Tributária)."
}'::jsonb),
...
-- Rode apenas uma vez (não há chave única em icp; re-rodar duplica linhas).
-- -----------------------------------------------------------------------------
