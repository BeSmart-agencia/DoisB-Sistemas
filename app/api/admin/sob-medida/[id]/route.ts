...
const schema = z.object({
  ...
  nome_projeto: z.string().min(1).optional(),
  descricao: z.string().optional().nullable(),
  tipo_sistema: z.string().optional().nullable(),
  status: z.enum(["proposta", "em_desenvolvimento", "entregue", "manutencao", "cancelado"]).optional(),
  ...
  valor_desenvolvimento: z.number().min(0).optional(),
  valor_recebido: z.number().min(0).optional(),
  mensalidade_valor: z.number().min(0).optional(),
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const { error } = await supabase!
    ...
    .update({ ...limpar(parsed.data), updated_at: new Date().toISOString() })
    .eq("id", params.id)

  if (error) {
    console.error("[sob-medida] Erro ao atualizar projeto:", error.message)
...
export async function DELETE(_request: Request, { params }: { params: { id: string } }) {
  ...
  const { error } = await supabase!.from("sob_medida_projetos").delete().eq("id", params.id)
  if (error) {
    console.error("[sob-medida] Erro ao excluir projeto:", error.message)
